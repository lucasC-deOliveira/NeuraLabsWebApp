...
const tipoLabel = (t: string) => NODE_TYPE_DISPLAY[t as keyof typeof NODE_TYPE_DISPLAY]?.label ?? t;
const relacaoLabel = (r: string) => RELATION_LABELS[r] ?? r.toLowerCase();
...
export function NodeInsightsModal({ open, onOpenChange, grafoId, nodeId, nodeLabel, onAdded }: NodeInsightsModalProps) {
  ...
  useEffect(() => {
    ...
    generateNodeInsights(grafoId, nodeId)
      ...
      .catch((e) => active && setError(e instanceof Error ? e.message : "Erro ao gerar insights."))
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const res = await addInsightsToGraph(grafoId, nodeId, chosen);
      toast.success(
        res.added > 0
          ? `${res.added} ${res.added === 1 ? "insight adicionado" : "insights adicionados"} ao grafo.`
          : "Nada foi adicionado (já existiam no grafo).",
      );
      onAdded?.();
      onOpenChange(false);
    } catch (e) {
      ...
      <DialogContent className="max-w-xl flex max-h-[85dvh] flex-col overflow-hidden gap-0">
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl border p-3 text-left transition-colors ${
                      on ? "border-primary bg-primary/5 ring-1 ring-primary/40" : "bg-card hover:border-primary/40"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${
                          CATEGORY_STYLE[ins.categoria] ?? "border-border bg-muted text-muted-foreground"
                        }`}
                      >
                        {ins.categoria}
                      </span>
        ...
        {!loading && !error && insights.length > 0 && (
          <div className="shrink-0 flex items-center gap-2 border-t pt-3">
            ...
            <Button className="flex-1 gap-2" onClick={handleAdd} disabled={selected.size === 0 || adding}>

...
// Literal string required — vi.mock factory is hoisted before variable initialization
vi.mock('@/lib/auth', () => ({
...
describe('getSubjects', () => {
  it('returns subjects belonging to the current user', async () => {
    ...
    expect(list.every((s) => s.id && s.name)).toBe(true)
  })
  ...
  it('does not leak subjects from other users', async () => {
    ...
    const list = await getSubjects()
    expect(list.some((s) => s.name === 'Segredo Alheio')).toBe(false)
  ...
  it('returns subjects sorted alphabetically by name', async () => {
    const names = (await getSubjects()).map((s) => s.name)

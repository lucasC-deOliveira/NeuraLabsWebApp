...
export function GraphChatModal({ open, onOpenChange, grafoId }: GraphChatModalProps) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const res = await chatWithGraph(grafoId, question, history);
      setMessages(prev => [
        ...prev,
        { role: "assistant", content: res.answer, referencedNodes: res.referencedNodes },
      ]);
    } catch (e) {
      setMessages(prev => [
        ...prev,
        { role: "assistant", content: `Erro: ${e instanceof Error ? e.message : "Falha na requisição."}` },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl h-[600px] flex flex-col gap-0 overflow-hidden">
        <DialogHeader className="shrink-0 pb-3">
          ...
          <DialogDescription>
            Pergunte sobre o conteúdo do seu grafo de conhecimento.
          </DialogDescription>
        </DialogHeader>
          ...
          {messages.length === 0 && !loading && (
            <div className="flex items-center justify-center h-full">
              <p className="text-sm text-muted-foreground text-center max-w-xs">
                Faça uma pergunta sobre o conteúdo do seu grafo...
              </p>
            </div>
          )}
                ...
                {msg.role === "assistant" && msg.referencedNodes && msg.referencedNodes.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1 px-1">
                    {msg.referencedNodes.map(n => (
                      <span
                        key={n.id}
                        className="text-[10px] bg-muted/60 border border-border rounded px-1.5 py-0.5 text-muted-foreground"
                      >
        ...
        <form onSubmit={handleSend} className="shrink-0 flex gap-2 p-3">
          <input

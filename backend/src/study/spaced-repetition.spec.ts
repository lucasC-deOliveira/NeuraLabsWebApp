...
describe('scheduleCard — carta nova (state = null) → fase LEARN', () => {
  it('again: volta ao passo 0, revisão em 1 min, dificuldade 10', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'LEARN', learningStep: 0, intervalo: 0, fatorEase: 2.5,
      dificuldade: 10, proximaRevisao: minLater(1), ultimaRevisao: NOW,
    });
  });
  ...
  it('hard: repete o passo atual (1 min), dificuldade 7', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'LEARN', learningStep: 0, intervalo: 0, fatorEase: 2.5,
      dificuldade: 7, proximaRevisao: minLater(1), ultimaRevisao: NOW,
    });
  });
  ...
  it('good: avança para o passo 1 (10 min), dificuldade 5', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'LEARN', learningStep: 1, intervalo: 0, fatorEase: 2.5,
      dificuldade: 5, proximaRevisao: minLater(10), ultimaRevisao: NOW,
    });
  });
  ...
  it('easy: gradua direto para REVIEW com intervalo de 4 dias, dificuldade 1', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 4, fatorEase: 2.5,
      dificuldade: 1, proximaRevisao: dayLater(4), ultimaRevisao: NOW,
    });
  });
});
...
describe('scheduleCard — LEARN no último passo gradua com good', () => {
  const learnStep1: ScheduleState = {
    fase: 'LEARN', learningStep: 1, intervalo: 0, fatorEase: 2.5,
    dificuldade: 5, proximaRevisao: NOW, ultimaRevisao: NOW,
  };

  it('good no passo 1 gradua para REVIEW com intervalo 1 dia', () => {
    const s = scheduleCard('good', learnStep1, NOW);
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 1, fatorEase: 2.5,
      dificuldade: 3, proximaRevisao: dayLater(1), ultimaRevisao: NOW,
    });
  });
});
...
describe('scheduleCard — fase REVIEW', () => {
  const review: ScheduleState = {
    fase: 'REVIEW', learningStep: 0, intervalo: 10, fatorEase: 2.5,
    dificuldade: 3, proximaRevisao: NOW, ultimaRevisao: NOW,
  };

  it('again: lapso → RELEARN, ease -0.2, intervalo *0.2, revisão em 1 min', () => {
    const s = scheduleCard('again', review, NOW);
    expect(s).toEqual<ScheduleState>({
      fase: 'RELEARN', learningStep: 0, intervalo: 2, fatorEase: 2.3,
      dificuldade: 10, proximaRevisao: minLater(1), ultimaRevisao: NOW,
    });
  });
  ...
  it('hard: ease -0.15, intervalo = round(10*1.2) = 12', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 12, fatorEase: 2.35,
      dificuldade: 7, proximaRevisao: dayLater(12), ultimaRevisao: NOW,
    });
  });
  ...
  it('good: intervalo = round(10*ease) = 25, ease inalterado', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 25, fatorEase: 2.5,
      dificuldade: 3, proximaRevisao: dayLater(25), ultimaRevisao: NOW,
    });
  });
  ...
  it('easy: ease +0.15, intervalo = round(10*ease*1.3) = 33', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 33, fatorEase: 2.65,
      dificuldade: 1, proximaRevisao: dayLater(33), ultimaRevisao: NOW,
    });
  });
...
describe('scheduleCard — fase RELEARN', () => {
  const relearn: ScheduleState = {
    fase: 'RELEARN', learningStep: 0, intervalo: 5, fatorEase: 2.0,
    dificuldade: 10, proximaRevisao: NOW, ultimaRevisao: NOW,
  };
  ...
  it('easy: volta para REVIEW mantendo o intervalo', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 5, fatorEase: 2.0,
      dificuldade: 1, proximaRevisao: dayLater(5), ultimaRevisao: NOW,
    });
  });
  ...
  it('good no passo 0: avança para passo 1 (10 min), continua RELEARN', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'RELEARN', learningStep: 1, intervalo: 5, fatorEase: 2.0,
      dificuldade: 3, proximaRevisao: minLater(10), ultimaRevisao: NOW,
    });
  });
  ...
  it('good no último passo gradua de volta para REVIEW', () => {
    ...
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 5, fatorEase: 2.0,
      dificuldade: 3, proximaRevisao: dayLater(5), ultimaRevisao: NOW,
    });
  });
...
describe('dbToState', () => {
  it('mapeia o registro e aplica defaults (fatorEase 2.5, learningStep 0)', () => {
    const s = dbToState({
      fase: 'REVIEW', learningStep: undefined as unknown as number,
      intervalo: 7, fatorEase: undefined as unknown as number,
      dificuldade: 3, proximaRevisao: NOW, ultimaRevisao: NOW,
    });
    expect(s).toEqual<ScheduleState>({
      fase: 'REVIEW', learningStep: 0, intervalo: 7, fatorEase: 2.5,
      dificuldade: 3, proximaRevisao: NOW, ultimaRevisao: NOW,
    });
  });
});

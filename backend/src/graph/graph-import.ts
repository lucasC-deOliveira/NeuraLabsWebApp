...
function displayName(n: ImportGraphNode): string {
  switch (n.tipo) {
    case 'ASSUNTO': case 'TOPICO': case 'CONCEITO': return (n.nome ?? '').trim();
    case 'FLASHCARD': return (n.pergunta ?? '').trim();
    case 'NOTA': return (n.titulo?.trim() || deriveNotaTitulo(n.conteudo ?? '')).trim();
    case 'TEXTO_BRUTO': return n.titulo?.trim() || 'Texto sem título';
    case 'BARALHO': return (n.titulo ?? n.nome ?? '').trim();
    default: return '';
  }
}
...
function validateFields(n: ImportGraphNode, pos: string): void {
  const need = (cond: boolean, msg: string) => { if (!cond) throw new BadRequestException(`${msg}${pos}.`); };
  switch (n.tipo) {
    case 'ASSUNTO': case 'TOPICO': case 'CONCEITO': need(!!n.nome?.trim(), '"nome" é obrigatório'); break;
    case 'FLASHCARD': need(!!n.pergunta?.trim(), '"pergunta" é obrigatória'); need(!!n.resposta?.trim(), '"resposta" é obrigatória'); break;
    case 'NOTA':
      ...
      break;
    case 'TEXTO_BRUTO': need(!!n.texto?.trim(), '"texto" é obrigatório'); break;
    case 'BARALHO': need(!!(n.titulo?.trim() || n.nome?.trim()), '"titulo" é obrigatório'); break;
  }
}
...
export async function runImportGraph(
  ...
  for (const [i, n] of nodes.entries()) {
    ...
    if (!IMPORT_NODE_TIPOS.includes(n.tipo)) throw new BadRequestException(`"tipo" inválido (${n.tipo})${pos}.`);
    refSet.add(ref);
  ...
  for (const [i, e] of edges.entries()) {
    ...
    if (e.origem === e.destino) throw new BadRequestException(`Aresta não pode ligar um nó a si mesmo${pos}.`);
    if (e.peso !== undefined && (!Number.isFinite(e.peso) || e.peso <= 0 || e.peso > 2)) throw new BadRequestException(`"peso" deve ser 0 a 2${pos}.`);
    if (!isRelationAllowed(to, td, e.relacao)) {
      const allowed = getAllowedRelations(to, td);
      throw new BadRequestException(`Relação "${e.relacao}" não permitida entre ${to} e ${td}${pos}.${allowed.length ? ` Permitidas: ${allowed.join(', ')}.` : ''}`);
    }
  }
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const createEntity = async (n: ImportGraphNode): Promise<string> => {
        switch (n.tipo) {
          case 'ASSUNTO': return (await tx.assunto.create({ data: { nome: n.nome!.trim(), descricao: n.descricao ?? null, usuarioId: userId } })).id;
      ...
      const existing = await tx.nodeConhecimento.findMany({ where: { grafoId, usuarioId: userId }, select: { id: true, tipoNode: true, referenciaId: true } });
      const refsByTipo = new Map<string, string[]>();
      for (const e of existing) (refsByTipo.get(e.tipoNode) ?? refsByTipo.set(e.tipoNode, []).get(e.tipoNode)!).push(e.referenciaId);
      const nameByRef = new Map<string, string>();
      for (const [tipo, ids] of refsByTipo) {
        if (!ids.length) continue;
        let rows: Array<{ id: string; name: string | null }> = [];
        if (tipo === 'ASSUNTO') rows = (await tx.assunto.findMany({ where: { id: { in: ids } }, select: { id: true, nome: true } })).map((r) => ({ id: r.id, name: r.nome }));
        else if (tipo === 'TOPICO') rows = (await tx.topico.findMany({ where: { id: { in: ids } }, select: { id: true, nome: true } })).map((r) => ({ id: r.id, name: r.nome }));
        else if (tipo === 'CONCEITO') rows = (await tx.conceito.findMany({ where: { id: { in: ids } }, select: { id: true, nome: true } })).map((r) => ({ id: r.id, name: r.nome }));
        else if (tipo === 'FLASHCARD') rows = (await tx.flashcard.findMany({ where: { id: { in: ids } }, select: { id: true, pergunta: true } })).map((r) => ({ id: r.id, name: r.pergunta }));
        else if (tipo === 'NOTA') rows = (await tx.nota.findMany({ where: { id: { in: ids } }, select: { id: true, titulo: true } })).map((r) => ({ id: r.id, name: r.titulo }));
        else if (tipo === 'TEXTO_BRUTO') rows = (await tx.textoBruto.findMany({ where: { id: { in: ids } }, select: { id: true, titulo: true } })).map((r) => ({ id: r.id, name: r.titulo }));
        else if (tipo === 'BARALHO') rows = (await tx.baralho.findMany({ where: { id: { in: ids } }, select: { id: true, titulo: true } })).map((r) => ({ id: r.id, name: r.titulo }));
        for (const r of rows) nameByRef.set(r.id, norm(r.name ?? ''));
      }
      const byName = new Map<string, { refId: string; nodeId: string }>();
      for (const e of existing) { const nm = nameByRef.get(e.referenciaId); if (nm) byName.set(`${e.tipoNode}::${nm}`, { refId: e.referenciaId, nodeId: e.id }); }
      ...
      for (const n of nodes) {
        ...
        if (reuse) { created.set(n.ref.trim(), reuse); reusedNodeCount++; continue; }
        const refId = await createEntity(n);
        const node = await tx.nodeConhecimento.create({ data: { grafoId, tipoNode: n.tipo as any, referenciaId: refId, usuarioId: userId } });
      ...
      const existingEdges = await tx.conhecimentoAresta.findMany({ where: { grafoId }, select: { nodeOrigemId: true, nodeDestinoId: true, tipoRelacao: true } });
      const seen = new Set(existingEdges.map((e) => `${e.nodeOrigemId}->${e.nodeDestinoId}->${e.tipoRelacao}`));
      let edgeCount = 0;
      for (const e of edges) {
        ...
        await tx.conhecimentoAresta.create({ data: { grafoId, nodeOrigemId: s.nodeId, nodeDestinoId: t.nodeId, tipoRelacao: e.relacao as any, peso: e.peso ?? 1 } });
        seen.add(key);
        edgeCount++;

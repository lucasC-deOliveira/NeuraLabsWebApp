...
class FakeAnalyticsSource implements FlashcardAnalyticsSource {
  constructor(
    ...
    private readonly problems: ProblemCardRow[] = [],
  ) {}
  ...
  async problemCardStats(): Promise<ProblemCardRow[]> {
...
describe('GetFlashcardAnalyticsUseCase', () => {
  it('assembles totals and every analytics section', async () => {
    ...
    const source = new FakeAnalyticsSource(
      ...
      [{ data: now, acertou: false, nivelConfianca: 5, tempoResposta: 3000, tipoErro: 'conceito' }],
      [{ pergunta: 'O que é X?', total: 4, wrong: 3 }],
    );
    ...
    expect(result.problemCards[0]).toMatchObject({
      pergunta: 'O que é X?',
      wrong: 3,
      accuracy: 25,
    });
